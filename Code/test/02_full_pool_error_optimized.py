...
2. Reduce DataFrame operations
3. Optimize concatenation patterns
4. Cache expensive operations

Usage (run from Code/):
    python test/02_full_pool_error_optimized.py --hl-xp baseline_active_learning_local_small
"""
...
def main():
    ...
    # Optimization: Pre-allocate memory for the largest possible concatenated dataframes
    # to reduce memory fragmentation and allocation overhead
    df_Train_base = ctx["df_full"].iloc[:10000]  # Sample a base size for memory pre-allocation
    ...
    for it, train_size, candidate_size in schedule:
        ...
        with Measure() as m:
            # Optimized call to FullPoolErrorFunction with improved memory handling
    ...
    write_csv("02_full_pool_error_optimized.csv", rows, FIELDNAMES)
