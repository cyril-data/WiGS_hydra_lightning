...
def AggregateResults(raw_results_dir, aggregated_results_dir):
    ...
    if not all_pkl_files:
        ...
        return
    dataset_basenames = sorted(
        list(set([os.path.basename(os.path.dirname(f)) for f in all_pkl_files]))
    )
    ...
    for data_name in dataset_basenames:
        ...
        for i, file_path in enumerate(result_files_for_dataset):
            ...
            if "InitialTrainIndices" in single_run_result[first_strategy_key]:
                all_initial_indices_list.append(
                    single_run_result[first_strategy_key]["InitialTrainIndices"]
                )
            ...
            for strategy, results in single_run_result.items():
                if strategy in aggregated_data:
                    ...
                    if "k_top_candidate" in results:
                        current_k_top = results["k_top_candidate"]
                        if k_top is None:
                            k_top = current_k_top
                        elif k_top != current_k_top:
                            print(
                                f"  > Warning: k_top_candidate varies across simulations for {data_name}. Using first value: {k_top}"
                            )
        ...
        for eval_type in eval_types:
            eval_metrics_save_dir = os.path.join(
                dataset_output_dir, f"{eval_type.lower()}_metrics"
            )
        ...
        for strategy in strategies:
            ...
            history_df.to_csv(
                os.path.join(history_save_dir, f"{strategy}_SelectionHistory.csv"),
        ...
        for strategy in strategies:
            ...
            if weight_data:
                ...
                if weight_df.notna().any().any():
                    ...
                    weight_df.to_csv(
                        os.path.join(weight_save_dir, f"{strategy}_WeightHistory.csv"),
                        index_label="Iteration",
                    )
                    files_saved_count += 1
...
if __name__ == "__main__":
    ...
    AggregateResults(
        raw_results_dir=RAW_RESULTS_DIR, aggregated_results_dir=AGGREGATED_RESULTS_DIR
    )
