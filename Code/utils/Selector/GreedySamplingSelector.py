...
class GreedySamplingSelector:
    ...
    def select(
        ...
        **kwargs,
        ...
        n_candidat = (SimulationConfigInputUpdated or {}).get("add_useful_params", {}).get(
            "subset_rand_candidat"
        )
        if n_candidat is not None and len(X_Candidate) > n_candidat:
            indices_random_iloc = np.random.choice(len(X_Candidate), size=n_candidat, replace=False)
